    ...
    <AdminShell title={editing ? 'Edit Place' : 'New Place'}>
      {loading ? <p>Loading...</p> : (
        <form className="editorial-form admin-form" onSubmit={submit}>
          <label>City<select value={form.city_id} onChange={(event) => update('city_id', event.target.value)} required><option value="">Choose city</option>{cities.map((city) => <option key={city.id} value={city.id}>{city.name}</option>)}</select></label>
          <label>Status<select value={form.status} onChange={(event) => update('status', event.target.value)}><option>draft</option><option>published</option></select></label>
          ...
          <label>Type<input value={form.type} onChange={(event) => update('type', event.target.value)} /></label>
          <label>Area<input value={form.area} onChange={(event) => update('area', event.target.value)} /></label>
          ...
          <label className="wide">Short description<textarea value={form.short_description} onChange={(event) => update('short_description', event.target.value)} /></label>
          <label className="wide">Why soulful<textarea value={form.why_soulful} onChange={(event) => update('why_soulful', event.target.value)} /></label>
